...
All configured backends are available during a session — you can switch models at any time with `/model` without losing your conversation history.
...
| `/model <name>` | Switch to a different model, carrying conversation history forward |
| `/models` | List all configured backends (active one marked) |
| `/exit` | End the session |

When switching models, the full conversation history is passed to the new model so it has complete context. If the new model is a local llamacpp backend it loads automatically before the next message.
