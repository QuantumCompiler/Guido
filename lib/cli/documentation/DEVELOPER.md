...
| `chat` | Interactive multi-turn session. All configured backends are loaded so the user can switch with `/model <name>` mid-session without losing history. Agentic loop when tools active, streaming otherwise. Logs one JSON file per session to `logs/chats/`. Supports `--resume`/`--continue` to reload a prior chat |
...
| `filterBackends(cfg, target, all)` | Restricts `cfg.Backends` to a single backend before initialization. Used by `complete` and `serve`; `chat` loads all backends to support mid-session model switching |
| `initializeBackends(h, cfg, tm, lazy)` | Instantiates and registers all backends in the config. `lazy=true` wraps llamacpp backends in `LazyLlamaCppBackend` — used by `chat` (all backends lazy, initial one eagerly warm-started via `warmModel`) and `serve` |
| `warmModel(ctx, name)` | Resolves a backend by name and calls `EnsureLoaded` if it is a lazy llamacpp backend. Used at `chat` startup and on `/model` switches. Returns false if the backend fails to load |
